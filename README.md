...
最后更新于: 2018-04-15 10:08:00 +0000

[分享到微博](http://service.weibo.com/share/share.php?url=http://phptherightway.p2hp.com&title=PHP+The+Right+Way+PHP+之道)  
[❤PHP中文网 ❤](http://www.p2hp.com/)
...
*   [欢迎阅读](#welcome)
    ...
    *   [分享与推广](#spread-the-word)
*   [入门指南](#getting_started)
    *   [使用当前稳定版本 (7.2)](#use_the_current_stable_version)
    *   [内置的 Web 服务器](#builtin_web_server)
    *   [Mac 安裝](#mac_setup)
    *   [Windows 安裝](#windows_setup)
*   [代码风格指南](#code_style_guide)
    
*   [语言亮点](#language_highlights)
    *   [编程范式](#programming_paradigms)
    *   [命名空间](#namespaces)
    *   [类型声明](#type-declaration)
    *   [PHP 标准库](#standard_php_library)
    *   [命令行接口](#command_line_interface)
    *   [Xdebug 调试工具](#xdebug)
*   [依赖管理](#dependency_management)
    *   [Composer 与 Packagist](#composer_and_packagist)
    *   [PEAR](#pear)
*   [开发实践](#coding_practices)
    *   [基础知识](#the_basics)
    *   [日期和时间](#date_and_time)
    *   [设计模式](#design_patterns)
    *   [使用 UTF-8 编码](#php_and_utf8)
    *   [国际化和本地化](#i18n_l10n)
*   [依赖注入](#dependency_injection)
    *   [基本概念](#basic_concept)
    *   [复杂的问题](#complex_problem)
    *   [容器](#containers)
    *   [延伸阅读](#further_reading)
*   [数据库](#databases)
    *   [MySQL 扩展](#mysql_extension)
    *   [PDO 扩展](#pdo_extension)
    *   [数据库交互](#databases_interacting)
    *   [数据库抽象层](#databases_abstraction_layers)
*   [使用模板](#templating)
    *   [好处](#templating_benefits)
    *   [原生 PHP 模板](#plain_php_templates)
    *   [编译模板](#compiled_templates)
    *   [延伸阅读](#templating_further_reading)
*   [错误与异常](#errors_and_exceptions)
    *   [错误](#errors)
    *   [异常](#exceptions)
*   [安全](#security)
    *   [Web 应用程序安全](#web_application_security)
...
创建一个类来放置上面的函数，你就得到了一个「Model」。创建一个简单的`.php`文件来存放表示逻辑，你就得到了一个「View」。这已经很接近 [MVC](http://code.tutsplus.com/tutorials/mvc-for-noobs--net-10488) — 一个大多数[框架](/#frameworks)常用的面向对象的架构。
...
*   [PHP 7 的异常和错误处理(推荐)](http://blog.p2hp.com/archives/4211)
...
最后的一个例子是接受外部输入来从文件系统中加载文件。这可以通过将文件名修改为文件路径来进行利用。你需要过滤掉`"/"`, `"../"`, [null 字符](http://php.net/security.filesystem.nullbytes)或者其他文件路径的字符来确保不会去加载隐藏、私有或者敏感的文件。

*   [学习更多数据过滤](http://php.net/book.filter)
*   [学习更多 `filter_var`](http://php.net/function.filter-var)
*   [学习更多 `filter_input`](http://php.net/function.filter-input)
...
如果你必须这样做，请你使用 PHP 7 的 [`allowed_classes`](https://secure.php.net/manual/en/function.unserialize.php) 选项来限制反序列化的对象类型。
...
> 将值设为 `-1` 将会显示出所有的错误，甚至包括在未来的 PHP 版本中新增加的类型和参数。 和 PHP 5.4 起开始使用的 `E_ALL` 是相同的。\- [php.net](http://php.net/function.error-reporting)
...
如果你追求高性能和高稳定性，可以为 Apache 选择与 nginx 类似的的 FPM 系统 [worker MPM](http://httpd.apache.org/docs/2.4/mod/worker.html) 或者 [event MPM](http://httpd.apache.org/docs/2.4/mod/event.html)，它们分别使用 mod\_fastcgi 和 mod\_fcgid。这种方式可以更高效的利用内存，运行速度也更快，但是配置也相对复杂一些。
...
*   [阅读更多 mod\_proxy\_fcgi](https://httpd.apache.org/docs/current/mod/mod_proxy_fcgi.html)
*   [配置 Apache 通过 mod\_proxy\_fcgi 使用 PHP-FPM](https://serversforhackers.com/video/apache-and-php-fpm)
...
[Puppet](https://puppet.com/) 拥有自定义语言和文件类型来管理服务和配置信息。支持 主从结构或者是 `无主结构`。在主从结构中，从属机器会在设定周期内更新主机上的配置信息。在无主架构中，你需要 `push` 推送修改到各个节点。
...
PHP 5.5 中自带了 opcode 缓存工具，叫做[Zend OPcache](http://php.net/book.opcache)，默认一般是开启的，请在 `phpinfo()` 输出中检查 [opcache.enable](http://php.net/manual/en/opcache.configuration.php#ini.opcache.enable) 关键词是否出现来确定是否开启。早期的版本也能通过 PECL 扩展来安装。
...
第二、第三个方法非常类似，和第一个方法一样使用一个 [@param](http://www.phpdoc.org/docs/latest/references/phpdoc/tags/param.html) 标记。第二、和第三个方法之间关键差別在注释区块使用／排除 [@return](http://www.phpdoc.org/docs/latest/references/phpdoc/tags/return.html) 标记。`@return void` 标记明确告诉我们没有返回值，而过去省略 `@return void` 声明也具有相同效果（沒有返回任何值）。
...
*   [惠新宸](https://weibo.com/laruence)
*   [韩天峰](https://weibo.com/hantianfeng)
*   [安正超](https://weibo.com/joychaocc)
*   [Easy](https://weibo.com/easy)
*   [sskaje](https://weibo.com/sskaje)
*   [李轩Lane](https://weibo.com/lanephp)
*   [Lenix](https://weibo.com/yyb8)
欢迎推荐大牛。
...
*   [现代PHP编程指导](http://blog.p2hp.com/archives/3634)
...
*   [SAE新浪云](http://www.sinacloud.com/sae.html)
*   [百度BAE](https://cloud.baidu.com/product/bae.html)
...
*   [热门的 PHP 框架](http://phptherightway.p2hp.com/page/?s=framework)
*   [关于如何正确使用PHP框架及如何选择框架?](http://blog.p2hp.com/archives/5170)
...
*   [Laravel 中文文档](http://laravel.p2hp.com/cndocs)
*   [Laravel 中文网](http://laravel.p2hp.com/) 我们是现代化 PHP 的拥护者
...
*   [请参考：程序包目录结构](https://github.com/php-pds/skeleton)
...
如果你住在较大的城市，附近应该就有 PHP 用户群。你可以通过基于 [PHP.ug](http://php.ug/) 的 [usergroup-list at php.net](http://php.net/ug.php) 这个地址找到当地的 PUG。也可以通过 [Meetup.com](http://www.meetup.com/find/) 或者使用搜索引擎 (i.e. [Google](https://www.google.com/search?q=php+user+group+near+me)) 搜索 `php user group near me` 。如果你住在比较小的城镇，当地也许还没有 PUG ，如果是这种情形，不妨就开始组建一个。
...
[ElePHPant](http://php.net/elephpant.php) 是PHP项目设计中有大象的漂亮吉祥物. 它最初是为1998的PHP项目设计的，由 [文森特·庞蒂埃(VincentPontier)](http://www.elroubio.net/)设计, \- 他是世界各地数千名大象的精神之父，10年后，可爱的毛绒大象玩具也诞生了。现在许多PHP会议上都有电子PHPants，还有许多PHP开发人员在他们的计算机上寻找乐趣和灵感.

[访问文森特·庞蒂埃](http://7php.com/elephpant/)
...
*   [Project Contributors](https://github.com/laravel-china/php-the-right-way/graphs/contributors)

[](http://creativecommons.org/licenses/by-nc-sa/3.0/)  
PHP: The Right Way by [Josh Lockhart](http://joshlockhart.com)  
is licensed under a [Creative Commons Attribution-NonCommercial-ShareAlike 3.0 Unported License](http://creativecommons.org/licenses/by-nc-sa/3.0/).  
